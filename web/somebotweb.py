...
def add_map_to_db(mapname, author, description, commit=True):
    ...
    maps is the table for data about maps (upload time, how many times tested,
    last tested)
...
def generate_preview(mapid):
    ...
    with open(os.path.join(app.config['PREVIEW_DIR'], str(mapid) + '.png'),
              'w') as f:
...
def generate_thumb(mapid):
    ...
    pre_x, pre_y = preview.size
    target_width = 250
    target_height = int(target_width * pre_y / float(pre_x))
    offset = (0, target_width / 2 - target_height / 2)
    if target_height > target_width:
        target_height = 250
        target_width = int(target_height * pre_x / float(pre_y))
        offset = (target_height / 2 - target_width / 2, 0)

    preview.thumbnail((target_width, target_height), Image.ANTIALIAS)
    centered_thumb = Image.new(preview.mode, size=(250, 250),
                               color=(0, 0, 0, 255))
    centered_thumb.paste(preview, offset)
    centered_thumb.save(
        os.path.join(app.config['THUMB_DIR'], str(mapid) + '.png'))


def recent_maps(author=None, page_limit=100, offset=0):
    if author:
        maps = Map.query.filter_by(author=author).offset(offset).limit(
            page_limit)
    else:
        maps = Map.query.order_by("upload_time desc").offset(offset).limit(
            page_limit)
    return maps
...
def upload_map():
    if request.method == 'POST':
        ...
        # Handle upload by dropzone, not sure how to specify filenames with
        # dropzone it sends just a list of files
        ...
        if layout and logic:
            ...
            if generate_test:
                return redirect(get_test_link(mapid))
            success = mapid >= 0
            if success:
        ...
        # This is a little hacky, recent_maps() returns a sqlite row,
        # but we need a list of mapnames
    ...
    if mapid and filetype and mapname:
        if filetype == "png":
            return send_from_directory(app.config['UPLOAD_DIR'],
                                       secure_filename(mapid + '.png'),
                                       attachment_filename=secure_filename(
                                           mapname + ".png"))
        elif filetype == "json":
            return send_from_directory(app.config['UPLOAD_DIR'],
                                       secure_filename(mapid + '.json'),
                                       as_attachment=True,
                                       attachment_filename=secure_filename(
                                           mapname + ".json"))
        else:
...
def search_db(query=None, mapname=None, author=None):
    ...
    if author and mapname:
        maps = Map.query.filter(Map.author.ilike(author)).filter(
            Map.mapname.ilike(mapname)).first()
    else:
        querystring = "%" + query + "%"
